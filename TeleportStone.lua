...
print(">> loading Teleport stone.lua")
...
local function ResetPlayer(player, flag, text)
	...
	player:SendAreaTriggerMessage("你需要重新登录角色，才能修改重新登录角色，才能修改"..text.."。")
...
local Stone={
	...
	SetHome=function(player)--设置当前位置为家
		local zb_x,zb_y,zb_z,mapId,areaId=player:GetX(),player:GetY(),player:GetZ(),player:GetMapId(),player:GetAreaId() 
		player:SetBindPoint(zb_x,zb_y,zb_z,mapId,areaId)
		player:SendBroadcastMessage("设置位置坐标 {x:"..zb_x..";y:"..zb_y..";z:"..zb_z..";mapid:"..mapId..";areaId:"..areaId.."} 为你的家。")
	end,
	...
	ResetFace=function(player)
		local zhubicount=60
		if  (player:GetItemCount(70002) >=zhubicount) then
			player:RemoveItem(70002, zhubicount)
			ResetPlayer(player, 0x8, "外貌")
		else 
		player:SendAreaTriggerMessage(GetItemLink(70002).."不足"..zhubicount.."个。")
		end
	end,
	ResetRace=function(player)
		local zhubicount=200
		if  (player:GetItemCount(70002) >=zhubicount) then
			player:RemoveItem(70002, zhubicount)
...
local Menu={
	[MMENU]={--主菜单
		...
		--{FUNC, "召唤幻化大师",	ST.SummonENPC,	GOSSIP_ICON_TABARD},
		{MENU, "职业技能训练师",MMENU+0x20,		GOSSIP_ICON_BATTLE, false,"点击后传送至训练师身旁。"},
		{MENU, "专业技能训练师",MMENU+0x30,		GOSSIP_ICON_BATTLE, false,"点击后传送至训练师身旁。"},
	...
	[TPMENU]={--传送菜单
		{TP, "|cFF800000 GM之岛 |r",			1, 16222.1,		16252.1,	12.5872,	0},
	...
	[TPMENU+0xa0]={--风景传送
		...
		{TP, "纳格兰雏形",		36, -1563.471802,618.916687,0.000416,	0},
		{TP, "卡拉赞地穴",		0, -11067,			-1820,		60,		0},
